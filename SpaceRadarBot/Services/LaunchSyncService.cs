...
public class LaunchSyncService
{
    ...
    private async Task SyncLaunches()
    {
        try
        {
            ...
            var upcomingLaunches = await FetchLaunchesFromApi($"{BaseUrl}/upcoming/?limit=20");
            ...
            if (allLaunches.Count > 0)
            {
                ...
                Console.WriteLine($"✅ [{DateTime.Now:HH:mm:ss}] Synced {allLaunches.Count} launches to database");
            }
            else
            {
